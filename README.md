...
- **1.1   Layout Theming**
    ...
    - **1.1.2 Setup process**
        - Creating themes
            ...
            - Within each theme folder, we create a `theme.jsx` component that imports the assets for that theme and returns them as one, this way by importing this single component lets say we call it (`theme-one.jsx`) into the app’s `layout.jsx` we are importing all the assets belonging to that theme.
            - To create the theme.jsx file for each theme, we make use of header.jsx
                - Since `header.jsx` already imports `main-menu.jsx` and `search-form.jsx` we just have it include the stylesheets as well to form a single component that then represents a theme
    ...
    - **1.1.3 Using theme**
        ...
        - The purpose of this particular `theme.jsx` file is to import all the available themes from their respective folders, then use a switch to decide which theme to render - depending on  what theme is recorded in the sellerConfigContext
        - This way instead of including this logic within the `layout.jsx` all we need to do is import this `theme.jsx`  into `layout.jsx`
    ...
    - The snippet below shows how the theme component is used within `layout.jsx`
        ...
        function Layout(props) {
             ...
             return(
                  <TopNotice />
        /*
        Remember that theme component houses the header component belonging to the rendered theme, hence why we place it here at the top
        */
                    **<Theme />** 
                     {props.children}
                    <Footer />
...
- **1.2   Color Pallete Manipulation**
    - **Purpose:** This feature allows us to dynamically manipulate the color palette used on the app. This includes the text colors, the component background colors, and button colors, etc
    - **1.2.1 Changing website colors**
        - To control which colors appear on the site as the primary, seconday colors, etc - we use the `ThemeColorPalleteManipulator` component
        ...
        - We allow website colors to be updated both via a color-picker as well as by entering a hexadecimal string representing the new color
        - Whenever a color is updated using the component, the sellerconfig context is updated accordingly
        - This state updated triggers a rerender and the app refreshes with the updated color palette stored within the sellerconfig context
    
    - **1.2.2 Using the defined color palette**
        - To allow the components to react to color changes in the color palette, we connect the colors used to the color palette state
        - We use these colors defined in the palette to give our components inline styling
        - The example below shows how to connect the background color of the top-notice component to the primary color chosen for the app
        - The above pattern is duplicated with every other component on the app that uses a color defined in the app’s color palette object
        ...
        - This ensures that if the user alters a color (this can be the primary color, secondary color, primary text color, etc ) in the color palette, all the components whose colors are connected to the palette values will rerender to use the updated colors
    
    - **Styling CSS pseudostates**
        - P**seudo-states** in CSS allows us to style depending on what state they are currently in
        ...
        - Since we want our elements to reflect changes in the color palette, including when these elements are in pseudo states - we need to connect these  **pseudo-states** to the color palette as well
        ...
        - Since it is impossible to alter the CSS **pseudo-states** of elements directly with javascript, we make use of the styled-components npm package to allow us to use functionality
        - We take components whose **pseudo-state** colors we want to connect to the color palette state and then convert the component into a styled-components
        - The following example shows a link that has been converted to a styled component and has had its **pseudo-state** connected to the color palette state
        ...
        <Link
             ...
             hoverColor={currentColorPallete.light_primary}
             activeColor={currentColorPallete.dark_primary}
             >
               My Account
        </Link>
        ```
